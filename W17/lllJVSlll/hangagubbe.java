...
public class hangagubbe {
		  ...
		  //Alla dessa 3 static int använder jag för att kunna visa informationen om spelen som spelats, 
		    ...
		    public static void main(String[] args) {
		        ...
		        String[] Ordlista = { "Skola", "Dator", "Läsk", "Panna",
		            "Klocka", "Kaffe", "Hörlurar", "Telefon", "Jordgubbe" };       // här finns ordlistan och koden som slumpar fram ordet.
		 
		        boolean spela = true; //används för att spela igen
		        while (spela) {
		            System.out.println("Välkommen till Hänga gubbe!");
		            char[] slumpatord =
		                Ordlista[random.nextInt(Ordlista.length)].toCharArray();
		            int AntalGissningar = slumpatord.length + 5; //här stötte jag på ett problem som var att antalet gissningar 
		           // var lika mpnga som ordet hade bokstäver, detta fixade jag simpelt med att addera en femma efter.
		            char[] SpelarGissning = new char[AntalGissningar];
		            ...
		            while (!GissatOrd && försök != AntalGissningar) {
		                ...
		                System.out.printf(
		                    "Du har %d försök kvar \n", AntalGissningar - försök);
		                System.out.println("Skriv en bokstav:");
		                char input = scanner.nextLine().charAt(0); //hela detta kodblocket visar upp nödvändig information till spelaren.
		                ...
		                if (input == '-') {
		                    ...
		                    for (int i = 0; i < slumpatord.length; i++) {
		                        if (slumpatord[i] == input) {
		                            SpelarGissning[i] = input; //detta kodblock kollar om ordet är fel eller om det inte är, om det inte är fel gör detta kodblock ingenting.
		                        }
		                    }
		            ...
		            String Spelaigen = scanner.nextLine(); // detta kodblock är kopplat till de andra 2 if satser som finns nedanför, det bestämmer om man vill spela igen eller om man vill avsluta.
		        ...
		        System.out.println("Vinster: " + Vinster + " Förluster: "
		            + Förluster + " Omgångar: " + Omgångar); // detta block är en fortsttning på de övre kodblocken, detta block visar information som samlats in under speltiden.
		    }
